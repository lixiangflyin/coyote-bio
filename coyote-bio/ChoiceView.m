...
- (id)initWithDelegate:(id)delegate
{
    ...
    if (self) {
        ...
        [_backgroundView setFrame:CGRectMake(0, 0, 187, 221)];
...
-(void)resetViewLocation:(int)buttonValue
{
    ...
    switch (buttonValue) {
        case 200:
        {
            _backgroundView.frame = CGRectMake(578, 227, 187, 221);
            //按钮控件全变灰色
            for(id tmpView in self.subviews){
                if([tmpView isKindOfClass:[UIButton class]]){
                    UIButton *button = (UIButton *)tmpView;
        ...
        case 201:
            _backgroundView.frame = CGRectMake(394, 345, 187, 221);
            //按钮控件全变灰色
            for(id tmpView in self.subviews){
                if([tmpView isKindOfClass:[UIButton class]]){
                    UIButton *button = (UIButton *)tmpView;
            ...
            break;
        case 202:
            _backgroundView.frame = CGRectMake(298, 470, 187, 221);
            //按钮控件全变灰色
            for(id tmpView in self.subviews){
                if([tmpView isKindOfClass:[UIButton class]]){
                    UIButton *button = (UIButton *)tmpView;
